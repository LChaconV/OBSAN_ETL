...
def run() -> None:
    ...
    config = load_transform_config("minerales_transform", CONFIG_PATH)
    ...
    df_golden = (
        df_golden.groupby(["id_mun", "year", "unit_measure", "mineral_resource"], dropna=False, as_index=False)["royalties_cop"]
        .sum()
    )
    

    save_fact_table(table_fact, run_name, fact_dir, config, "minerales")
    save_fact_table(df_golden, run_name, fact_dir_golden, config, "minerales")
