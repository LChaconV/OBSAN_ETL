...
TRANSFORM_CONFIG_PATH = PROJECT_ROOT / "config" / "transform" / "pme_por_genero_transform.yaml"

create_table_sql = """
CREATE TABLE IF NOT EXISTS mp_gender (
    id_mp_gender SERIAL PRIMARY KEY,
    year INTEGER,
    id_dept VARCHAR(10),
    id_gender INTEGER,
...
create_index_sql = """
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_mp_gender
ON mp_gender (year, id_dept, id_gender);
"""
def run() -> None:
    load_parquet_to_postgres(
        transform_config_path=TRANSFORM_CONFIG_PATH,
        config_key="pme_por_genero_transform",
        table_name="mp_gender",
        state_key="pme_por_genero_load",
        log_file_name="load_pme_por_genero.log",
