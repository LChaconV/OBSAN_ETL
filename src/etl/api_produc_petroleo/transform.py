...
CONFIG_PATH = PROJECT_ROOT / "config" / "transform" / "produc_petroleo_transform.yaml"
LOG_DIR = PROJECT_ROOT / "logs"


def get_config() -> dict:
    return load_transform_config("produc_petroleo_transform", CONFIG_PATH)
...
def build_golden(table_fact: pd.DataFrame) -> pd.DataFrame:
    df_golden = (
        table_fact
        .groupby(["latitud", "longitud", "year"], dropna=False, as_index=False)["produc_bls"]
        .sum()
    )
    ...
    return df_golden[["year", "produc_bls", "geometry"]]
...
def run() -> None:
    ...
    save_fact_table(table_fact, run_name, fact_dir, config, "produc_petroleo")
    save_fact_table(df_golden, run_name, fact_dir_golden, config, "produc_petroleo")
