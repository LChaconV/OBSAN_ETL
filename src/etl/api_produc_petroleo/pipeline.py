"""
pipeline.py — Producción de petróleo: extracción, transformación y carga por año.
...
Estrategia:
  ...
  3. Para cada año a procesar: descarga el año completo desde la API,
     agrega al grano campo (latitud/longitud), y hace UPSERT en oil_production.
...
CREATE TABLE IF NOT EXISTS oil_production (
    ...
    produc_bls  FLOAT,
    geometry    GEOMETRY,
    CONSTRAINT ux_oil_production UNIQUE (year, geometry)
);
"""
...
def _upsert_year(df_golden: pd.DataFrame, engine) -> None:
    ...
    with engine.begin() as conn:
        write_frame_to_db(df_golden, table_name="temp_oil_production", conn=conn, if_exists="replace")
        upsert_sql = build_upsert_query(
            table_name="oil_production",
            temp_table="temp_oil_production",
            insert_columns=cols,
            conflict_columns=CONFLICT_COLUMNS,
            update_columns=UPDATE_COLUMNS,
        )
        conn.execute(text(upsert_sql))
        conn.execute(text("DROP TABLE IF EXISTS temp_oil_production;"))
...
def _run_steps(**kwargs) -> None:
    ...
    logging.info("=== Inicio pipeline producción de petróleo ===")
    ...
    logging.info("=== Pipeline producción de petróleo finalizado ===")
