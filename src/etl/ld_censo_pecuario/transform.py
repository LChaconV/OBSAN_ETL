...
def load_transform_config(key: str) -> dict:
    ...
    if "censo_pecuario_transform" not in config:
        raise KeyError("No existe la clave 'censo_pecuario_transform' en censo_pecuario_transform.yaml")

    if key not in config["censo_pecuario_transform"]:
        raise KeyError(f"No existe la clave '{key}' en censo_pecuario_transform.yaml")

    return config["censo_pecuario_transform"][key]
