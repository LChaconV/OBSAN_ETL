...
def _load_config() -> dict:
    return load_yaml(CONFIG_PATH)["oro_aluvion"]
...
def find_years_to_process(last_checkpoint: str | None) -> list[int]:
    ...
    - Siempre incluye el año calendario actual.
...
def find_all_years_in_api() -> list[int]:
    ...
    rows = fetch_api_page(
        base_url=base_url,
        params={"$select": "a_o", "$group": "a_o", "$limit": 1000},
        timeout=timeout,
...
if __name__ == "__main__":
    ...
    print("\n── 1. Todos los años disponibles en la API ──")
    years = find_all_years_in_api()
    print(f"   Años: {years}")

    print("\n── 2. Años a procesar (sin checkpoint previo) ──")
    years_to_process = find_years_to_process(last_checkpoint=None)
    print(f"   Años: {years_to_process}")

    print("\n── 3. Checkpoint máximo actual ──")
    checkpoint = get_api_max_updated_at()
    print(f"   max(:updated_at) = {checkpoint}")

    for year_prueba in years_to_process:
        print(f"\n── 4. Descarga de datos para el año {year_prueba} ──")
        df = fetch_year_data(year_prueba)
