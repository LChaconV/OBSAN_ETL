...
def run(**kwargs) -> Path | None:
    ...
    setup_logging(LOG_DIR, "erradicacion_cultivos_etl.log")
    logging.info("Iniciando extracción de Erradicación de Cultivos de Coca")

    config = load_source_config("erradicacion_cultivos", CONFIG_PATH)
    state = load_state("erradicacion_cultivos", STATE_PATH)
