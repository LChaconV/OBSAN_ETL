...
SOURCES_CONFIG_PATH = PROJECT_ROOT / "config" / "transform" / "red_vial_transform.yaml"
...
def ensure_db_infrastructure(engine) -> None:
    ...
    logging.info("Validando infraestructura de base de datos para dim_road_network")
    with engine.begin() as conn:
        conn.execute(text("CREATE EXTENSION IF NOT EXISTS postgis;"))
 
        conn.execute(text("""
            CREATE TABLE IF NOT EXISTS dim_road_network(
                id_road VARCHAR(10),
        ...
        conn.execute(text("""
            ...
            ON dim_road_network USING GIST (geometry);
        """))
...
def main() -> None:
    ...
    setup_logging(LOG_DIR, "load_red_vial.log")
    logging.info("Iniciando proceso de carga: dim_road_network")
    ...
    try:
        ...
        db_state = load_state("red_vial", STATE_DB_PATH)
        ...
        silver_path = PROJECT_ROOT / sources_config["red_vial"]["source"]["silver_fact_dir"]
        ...
        with engine.begin() as conn:
            ...
            query_existing = text("SELECT id_road FROM dim_road_network")
            existing_ids = pd.read_sql(query_existing, conn)["id_road"].tolist()
            
            # Identificación de registros para inserción
            df_to_load = gdf_source[~gdf_source["id_road"].isin(existing_ids)]

            if not df_to_load.empty:
                logging.info("Detectados %d registros nuevos para inserción.", len(df_to_load))
                
                # Carga efectiva a PostgreSQL/PostGIS
                df_to_load.to_postgis("dim_road_network", conn, if_exists="append", index=False)
                
                # Actualización de estado de base de datos post-transacción exitosa
                current_max_id = df_to_load["id_road"].max()
                update_state(
                    key="red_vial",
                    incremental_value=current_max_id,
                    incremental_column="id_road",
